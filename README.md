...
本平台可**实时监听抖音直播间弹幕**，通过 DrissionPage 操控浏览器注入 MutationObserver，将每一条弹幕（含 emoji 表情）实时抓取，经 jieba 分词 + 情绪分析后，通过 WebSocket 推送到 Vue3 前端大屏，以 ECharts 图表实时可视化展示。

适用于：直播运营分析、弹幕舆情监控、数据可视化学习。
